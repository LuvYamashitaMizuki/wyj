...
argparser.add_argument('--mixture_coefficients', default="1;1;1", type=str, required=False,
                       help="logits, separate by ';'")
...
class ElmoWordFromTextEncoder:
    ...
    * allennlp 0.9.0, pytorch > 1.2.0
    ...
    def __init__(self, valid_vocab=None):
        ...
        self.model = Elmo(options_file, weight_file, 1, dropout=0,
                          scalar_mix_parameters=[float(x) for x in args.mixture_coefficients.split(";")]).to(device)
        self.device = device
...
def init():
    ...
    word_to_file, _, files = preprocess.create_vocab_and_files(stopwords, dataset=args.data, preprocess=5, type="train",
                                                               vocab=set())
