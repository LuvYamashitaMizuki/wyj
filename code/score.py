...
def main():
    ...
    stopwords = set(
        line.strip() for line in open('C:/Users/chel/PycharmProjects/Cluster-Analysis-master/stopwords_en.txt'))

    vocab = create_global_vocab(args.vocab)

    train_word_to_file, train_w_to_f_mult, files = create_vocab_and_files(stopwords, args.dataset, args.preprocess,
                                                                          "train", vocab)
    files_num = len(files)
    ...
    if args.entities == "word2vec":
        model = gensim.models.KeyedVectors.load_word2vec_format(
            'C:/Users/chel/PycharmProjects/Cluster-Analysis-master/model/GoogleNews-vectors-negative300.bin',
            binary=True)
        intersection, words_index_intersect = find_intersect(model.vocab, train_w_to_f_mult, model, files_num,
                                                             args.entities, args.doc_info)
        ...
        data, word_index = read_entity_file(args.entities_file, args.id2name, train_word_to_file, args.entities,
                                            elmomix=elmomix)
        intersection, words_index_intersect = find_intersect(word_index, train_w_to_f_mult, data, files_num,
                                                             args.entities, args.doc_info)
    ...
    dev_word_to_file, dev_word_to_file_mult, dev_files = create_vocab_and_files(stopwords, args.dataset,
                                                                                args.preprocess, "valid", vocab)
    dev_files_num = len(dev_files)

    test_word_to_file, test_word_to_file_mult, test_files = create_vocab_and_files(stopwords, args.dataset,
                                                                                   args.preprocess, "test", vocab)
    ...
    for topics in args.num_topics:
        ...
        while rand < NSEEDS:
            ...
            try:
                top_k_words, top_k = cluster(args.clustering_algo, intersection, words_index_intersect, topics,
                                             args.rerank, weights, args.topics_file, rand)
            except:
                ...
                top_k_words, top_k = cluster(args.clustering_algo, intersection, \
                                             words_index_intersect, topics, args.rerank, weights, args.topics_file,
                                             new_rand)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--elmomix", type=str, default="",
                        help="elmomix coefficients, separated by ';', should sum to 1")
