...
class SBertWordFromTextEncoder:

    def __init__(self, valid_vocab=None):
        self.device = device
        self.sent_tokenizer = nltk.data.load('tokenizers/punkt/english.pickle')
        self.model, self.sbert_tokenizer = self.load_sbert_models()
        self.w2vb = {} #embeds_sum
        self.w2vc = {} #counts这是什么？
        self.compounds = set()#不知道有没有这个
    ...
    def test_encoder(self):
        ...
        input_ids = torch.tensor([self.sbert_tokenizer.encode('Here is some text to \
            encode')]).to(self.device)
        last_hidden_states = self.model(input_ids)[0][0]
        print("SBert models are working fine\n")


    def load_sbert_models(self):
        #sentence_model = SentenceTransformer("typeform/distilbert-base-uncased-mnli")
        model_class = SentenceTransformer
        pretrained_weights = "typeform/distilbert-base-uncased-mnli"
    ...
    def encode_docs(self, docs=[], save_fn=""):
        ...
        with torch.no_grad(): 
            for i, doc in enumerate(docs):
                ...
                for sent in sents:
                    words = self.sbert_tokenizer.tokenize(sent)
                    ...
                    if len(words) > 500:
                        ...
                        if not words == self.sbert_tokenizer.tokenize(detok):
                            pdb.set_trace()
                    else:
                        new_sents = [sent]
                    ...
                    for sent in new_sents:
                        if len(new_sents) > 1:
                            words = self.sbert_tokenizer.tokenize(sent)
                        try:
                            input_ids = torch.tensor([self.sbert_tokenizer.encode(sent)]).to(self.device)
                        except:
                            pdb.set_trace()
...
def init():
    ...
    word_to_file = {}
    word_to_file, _, files = preprocess.get_dataset(dataset=args.data, type="train")###这里函数要改
    
    valid_vocab = word_to_file.keys()

    ### this is what you care about
    encoder = SBertWordFromTextEncoder(valid_vocab=valid_vocab)
